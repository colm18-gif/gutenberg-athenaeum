...
The in-library Support Box now opens a support dialog linked to Stripe. `Support Box Opened` records that dialog opening; `Stripe Support Opened` records a click on its checkout link. Both are interest signals, not payments. Register these two event goals in Plausible to see them in the dashboard. Count revenue only from verified successful Stripe payments. No visitor identifier, cookies, IP, coordinates, search text, book titles, conversation choices or personal data are deliberately sent as custom event properties. Existing localStorage progress is unchanged and is not sent to Plausible.
...
## Monetisation measurement update — 15 September 2026

The current Site source was cloned cleanly at `85ded587d9bef8a46fa366992dc864b09fa68a02`; local tag `monetisation-baseline-2026-09-15` preserves this rollback point. The two Support Box events were already emitted by `game.js` but silently filtered by the `analytics.js` event allowlist. This update permits only those two fixed names and adds focused checks to `analytics.test.cjs`. Plausible account sign-in and goal registration are needed to verify ingested counts. The Stripe checkout suggests €3 but allows the visitor to choose the amount. No financial setting was changed.
