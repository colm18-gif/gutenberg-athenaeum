...
test('Nautilus hull and plinth are blocked but its surrounding walkways and other floor levels remain open',()=>{
  ...
  assert(!runtime.walk(170,46));assert(!runtime.walk(171.9,46));assert(runtime.walk(172.7,46));assert(runtime.walk(170,47.6));assert(runtime.walk(170,44.4));runtime.floorHeight=()=>3;assert(runtime.walk(170,46));
});
