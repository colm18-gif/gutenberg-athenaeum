(function(){
  ...
  window.createHighStaircase=function(options){
    ...
    function allowed(x,z){if(rocketTrip)return false;if(onMoon(x,z))return Math.hypot(x-mx,z-mz)<moonRadius-1;if(inTransit(x,z))return Math.hypot(x-tx,z-tz)<2.45;const pathStep=closestStep(x,z);if(pathStep)return true;const radius=player.radius||.42,samples=[[0,0],[radius,0],[-radius,0],[0,radius],[0,-radius],[radius*.707,radius*.707],[-radius*.707,radius*.707],[radius*.707,-radius*.707],[-radius*.707,-radius*.707]];const summit=Math.hypot(x-cx,z-cz)<4.35&&player.pos.y>topY-2,bottom=Math.hypot(x-bottomX,z-bottomZ)<2.2&&player.pos.y<2;if(summit||bottom)return samples.every(([dx,dz])=>summit?Math.hypot(x+dx-cx,z+dz-cz)<4.48:Math.hypot(x+dx-bottomX,z+dz-bottomZ)<2.38);return false}
