(function(){
  ...
  window.createHighStaircase=function(options){
    ...
    const cx=224,cz=30,topY=30,steps=180,turns=3.2,outerRadius=12,innerRadius=5.15,mx=340,mz=30,moonRadius=18,tx=380,tz=30;
    ...
    const entrance=new THREE.Group();entrance.name='high-stair-entrance';entrance.position.set(entranceX,0,entranceZ);scene.add(entrance);
    ...
    cylinder(6.8,6.8,.38,48,MAT.wood,cx,topY-.19,cz);
    ...
    const summitRug=add(new THREE.CircleGeometry(5.9,48),new THREE.MeshStandardMaterial({map:rugTex,roughness:.92}),cx,topY+.012,cz);summitRug.rotation.x=-Math.PI/2;
    for(let i=0;i<10;i++){if(i===5)continue;const a=i/10*Math.PI*2,r=6.25,x=cx+Math.cos(a)*r,z=cz+Math.sin(a)*r,shelf=box(2.15,3.2,.45,MAT.darkWood,x,topY+1.6,z);shelf.rotation.y=-a+Math.PI/2;for(const yy of [topY+.58,topY+1.48,topY+2.38]){const ledge=box(2.05,.1,.54,MAT.brass,x,yy,z);ledge.rotation.y=shelf.rotation.y}}
    const dome=add(new THREE.SphereGeometry(7.05,32,16,0,Math.PI*2,0,Math.PI/2),blueGlass,cx,topY,cz);dome.material.side=THREE.DoubleSide;
    ...
    wanted.forEach((book,index)=>{
      const a=-Math.PI*.72+index/(Math.max(1,wanted.length-1))*Math.PI*1.44,r=3.65,x=cx+Math.cos(a)*r,z=cz+Math.sin(a)*r;
    ...
    const summitTitle=add(new THREE.PlaneGeometry(3.8,.56),new THREE.MeshStandardMaterial({map:summitTitleTex,roughness:.75}),cx,topY+2.1,cz-6.52);summitTitle.userData={type:'high-stair-inscription',title:'The Last Landing',author:'The catalogue calls this the top. The staircase disagrees.',action:'READ'};interactables.push(summitTitle);
    ...
    const summitExit=box(2.3,3.7,.22,MAT.darkWood,cx-6.45,topY+1.85,cz);summitExit.rotation.y=Math.PI/2;summitExit.userData=summitExitData;interactables.push(summitExit);
    for(const zz of [-1.28,1.28])box(.2,4.05,.3,MAT.brass,cx-6.47,topY+2.02,cz+zz);
    const exitLintel=box(.22,.2,2.75,MAT.brass,cx-6.47,topY+4.02,cz);exitLintel.userData=summitExitData;interactables.push(exitLintel);
    ...
    const summitExitSign=add(new THREE.PlaneGeometry(2.5,.3),new THREE.MeshStandardMaterial({map:summitExitTex,emissive:0x6b461e,emissiveIntensity:.45}),cx-6.58,topY+3.35,cz);summitExitSign.rotation.y=Math.PI/2;summitExitSign.userData=summitExitData;interactables.push(summitExitSign);
    const summitExitGlow=new THREE.PointLight(0xffbd72,14,8,2);summitExitGlow.position.set(cx-5.35,topY+2.8,cz);root.add(summitExitGlow);
    ...
    function floorAt(x,z){if(onMoon(x,z)||inTransit(x,z))return 0;if(!contains(x,z))return null;const r=Math.hypot(x-cx,z-cz);if(r<6.7&&player.pos.y>topY-2)return topY;if(Math.hypot(x-bottomX,z-bottomZ)<2.7&&player.pos.y<2)return 0;return closestStep(x,z)?.y??null}
    function allowed(x,z){if(rocketTrip)return false;if(onMoon(x,z))return Math.hypot(x-mx,z-mz)<moonRadius-1;if(inTransit(x,z))return Math.hypot(x-tx,z-tz)<2.45;const pathStep=closestStep(x,z);if(pathStep)return true;const radius=player.radius||.42,samples=[[0,0],[radius,0],[-radius,0],[0,radius],[0,-radius],[radius*.707,radius*.707],[-radius*.707,radius*.707],[radius*.707,-radius*.707],[-radius*.707,-radius*.707]];const summit=Math.hypot(x-cx,z-cz)<6.15&&player.pos.y>topY-2,bottom=Math.hypot(x-bottomX,z-bottomZ)<2.2&&player.pos.y<2;if(summit||bottom)return samples.every(([dx,dz])=>summit?Math.hypot(x+dx-cx,z+dz-cz)<6.35:Math.hypot(x+dx-bottomX,z+dz-bottomZ)<2.38);return false}
