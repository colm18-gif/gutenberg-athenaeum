(function(){
  ...
  window.createHighStaircase=function(options){
    ...
    // stairwell opening above it, ringed by a brass balustrade that stays open only at the top step.
    const wellSteps=stepPath.filter(step=>step.y>=topY-2.5&&step.y<=topY-.25),wellFrom=wellSteps[0],wellTo=wellSteps[wellSteps.length-1],WELL_IN=1.0,WELL_OUT=.9;
    ...
    const rugShape=new THREE.Shape();rugShape.absarc(0,0,7.1,0,Math.PI*2,false);const rugHole=new THREE.Path();rugHole.absarc(0,0,2.25,0,Math.PI*2,true);rugShape.holes.push(rugHole,wellPath());
    ...
    function floorAt(x,z){if(onMoon(x,z)||inTransit(x,z))return 0;if(!contains(x,z))return null;const r=Math.hypot(x-cx,z-cz);if(inStairwell(x,z))return closestStep(x,z)?.y??null;if(r<9.4&&player.pos.y>topY-2)return topY;if(Math.hypot(x-bottomX,z-bottomZ)<2.7&&player.pos.y<2)return 0;return closestStep(x,z)?.y??null}
    function allowed(x,z){if(rocketTrip)return inTransit(x,z)&&Math.hypot(x-tx,z-tz)<2.45;if(onMoon(x,z))return Math.hypot(x-mx,z-mz)<moonRadius-1;if(inTransit(x,z))return Math.hypot(x-tx,z-tz)<2.45;const pathStep=closestStep(x,z);/* In the stairwell only the stair itself can be walked on, and only from a step at about the same height. */if(inStairwell(x,z))return !!pathStep&&Math.abs(pathStep.y-player.pos.y)<.7;if(pathStep)return true;const radius=player.radius||.42,samples=[[0,0],[radius,0],[-radius,0],[0,radius],[0,-radius],[radius*.707,radius*.707],[-radius*.707,radius*.707],[radius*.707,-radius*.707],[-radius*.707,-radius*.707]];const summit=Math.hypot(x-cx,z-cz)<9.05&&player.pos.y>topY-2,bottom=Math.hypot(x-bottomX,z-bottomZ)<2.2&&player.pos.y<2;if(summit||bottom)return samples.every(([dx,dz])=>summit?Math.hypot(x+dx-cx,z+dz-cz)<9.08:Math.hypot(x+dx-bottomX,z+dz-bottomZ)<2.38);return false}
