...
(()=>{
  ...
  window.createNightTrain=function({ride=()=>{},whistle=null,steam=null,THREE,scene,MAT,player,collider,colliders,nearbyColliders=(x,z)=>colliders,interactables,canvasTexture,wrapText,coverTexture,books,fogBook=window.ATHENAEUM_FOG_BOOK,performanceZones,rememberLights,move,notice,home,sound:playCue=()=>{},modelTemplate,isLowBandwidth,registerSeat,camera,chooseDestination}){
    ...
    function build(){if(built)return;built=true;
      ...
      // Rain on the glass: beads that cling while the carriage waits and streak backwards once it runs.
      if(THREE.ShaderMaterial){rainGlass={uniforms:{uTime:{value:0},uSpeed:{value:0}}};const rainMaterial=new THREE.ShaderMaterial({uniforms:rainGlass.uniforms,transparent:true,depthWrite:false,vertexShader:'varying vec2 vUv;void main(){vUv=uv;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}',fragmentShader:`uniform float uTime,uSpeed;varying vec2 vUv;
        float hash(vec2 p){return fract(sin(dot(p,vec2(12.9898,78.233)))*43758.5453);}
        void main(){vec2 uv=vUv*vec2(110.,9.);float a=0.;
          for(int layer=0;layer<2;layer++){float l=float(layer);vec2 g=uv*(1.+l*.7)+vec2(uTime*uSpeed*(1.4+l)*1.6,-uTime*(.05+uSpeed*.35)*(1.+l*.5));vec2 id=floor(g),f=fract(g)-.5;float h=hash(id+l*17.);
            vec2 d=f-vec2(h-.5,fract(h*7.3)-.5)*.6;float bead=smoothstep(.07,.015,length(d*vec2(1.,1.4+uSpeed*3.)));
            float trail=smoothstep(.025,.0,abs(d.y+d.x*uSpeed*1.5))*smoothstep(.5,-.1,d.x)*step(0.,d.x+.45)*uSpeed*.35;a+=(bead+trail)*step(.62,h);}
          gl_FragColor=vec4(vec3(.55,.6,.66),min(.22,a*.28));}`});
      for(const x of [217.96,223.04]){const pane=new THREE.Mesh(new THREE.PlaneGeometry(19.4,1.8),rainMaterial);pane.position.set(x,2.35,-20);pane.rotation.y=x<220?Math.PI/2:-Math.PI/2;pane.renderOrder=3;car.add(pane)}}
      // Lamps and lit platforms outside sweep warm light through the carriage now and then.
      if(THREE.PointLight){passLight=new THREE.PointLight(0xffc27a,0,9,2);passLight.position.set(217,2.6,-10);car.add(passLight)}
    ...
    function cancel(){looping=false;travelling=false;elapsed=0;nextWheel=0;alightCleared=false;arrivalPending=false;journeyHud.hide()}
    ...
    function offerDestinations(){if(travelling&&looping){if(chooseDestination)chooseDestination(stops.map((stop,index)=>({index,name:destinationName(stop),line:'Leave the loop and stop here.'})),index=>leaveLoop(index));else leaveLoop((stopIndex+1)%stops.length);return}if(travelling){notice('“We are between stations. I will call the stop when we arrive.”',6);return}if(chooseDestination){chooseDestination([{index:'loop',name:'Keep travelling',line:'Round and round the line all night, with no stops. Ask the conductor or use the route map when you want to get off.'},...destinations()],index=>depart(index))}else depart((stopIndex+1)%stops.length)}
    // The loop: the train runs on through the night without stopping, for as long as the reader likes.
    function destinationName(stop){const name=stop.key==='fog-platform'&&!fogVisited?'The stop with no name':stop.name;return name.charAt(0).toUpperCase()+name.slice(1)}
    function startLoop(){if(travelling)return;looping=true;targetStop=stopIndex;departedFrom=stopIndex;travelling=true;elapsed=0;alightCleared=false;arrivalPending=false;controls.alight.userData.action='DOOR LOCKED · TRAIN MOVING';controls.alight.userData.author='The train is running the loop. Ask the conductor to stop at a station.';journeyHud.show(stops[departedFrom].name.replace(/^the /,''),'the loop');if(!whistle?.()){playCue(523,.55,'sine',.05);later(()=>playCue(659,.9,'sine',.045),380)}notice('“Round the loop it is. We shall not stop until you ask.” The whistle sounds and the platform slides away.',8)}
    function leaveLoop(index){if(!looping)return;looping=false;targetStop=index;elapsed=Math.min(elapsed,14);journeyHud.show('the loop',stops[index].name.replace(/^the /,''));notice('“'+stops[index].name+', next stop.” The conductor makes a note, and the train runs on towards the station.',7)}
    ...
    const journeyHud=(()=>{let el=null;const doc=typeof document!=='undefined'?document:null;function ensure(){if(el||!doc)return el;el=doc.createElement('div');el.id='railHud';el.setAttribute('aria-live','off');el.innerHTML='<div class="flight-phase"></div><div class="flight-track"><span class="flight-from"></span><div class="flight-line"><i></i><b>●</b></div><span class="flight-to"></span></div><div class="flight-readout"></div>';doc.body.appendChild(el);return el}
      return {show(from,to){const h=ensure();if(!h)return;h.querySelector('.flight-from').textContent=from.toUpperCase();h.querySelector('.flight-to').textContent=to.toUpperCase();h.classList.add('active')},hide(){el?.classList.remove('active')},update(e,arrived,loop){if(!el)return;if(loop){const k=(e-5)/19%1;el.querySelector('.flight-phase').textContent='ON THE LOOP · NO STOPS';el.querySelector('.flight-line i').style.width=`${k*100}%`;el.querySelector('.flight-line b').style.left=`${k*100}%`;el.querySelector('.flight-readout').textContent=`${Math.round(speedAt(e)*42)} MPH · ASK THE CONDUCTOR TO GET OFF`;return}const k=Math.min(1,e/RUN),eased=k*k*(3-2*k);el.querySelector('.flight-phase').textContent=arrived?'ARRIVED · THE DOOR IS OPEN':e<5?'PULLING AWAY':e<26?'UNDER WAY':'BRAKING FOR THE PLATFORM';el.querySelector('.flight-line i').style.width=`${eased*100}%`;el.querySelector('.flight-line b').style.left=`${eased*100}%`;el.querySelector('.flight-readout').textContent=arrived?'MIND THE STEP':`${Math.round(speedAt(e)*42)} MPH · ${Math.max(0,Math.ceil(RUN-e))} S TO ARRIVAL`}}})();
    function interact(object){if(object?.userData?.type!=='night-railway')return false;const key=object.userData.key,zone=zoneAt(player.pos.x,player.pos.z)?.key;
      if(key==='entrance'){build();cancel();stopIndex=0;targetStop=0;move(213,-20,-Math.PI/2);notice('The conductor waits beside the reading carriage. Ask him when you are ready to travel.',7)}
        ...
        if(zone==='carriage'){
          if(travelling&&looping)offerDestinations();
          else if(travelling){const lines=['“Mind the sway. The books travel better than some passengers.”','“The lights ahead belong to '+stops[targetStop].name+'. I will call it when we arrive.”','“Take a seat by the window if you like. The country is best read at speed.”'];notice(lines[conductorTalk++%lines.length],7)}
    ...
    function update(t,dt,reduced,active,sound){sync();const zone=zoneAt(player.pos.x,player.pos.z)?.key;if(!zone)return false;if(fogBookMesh&&!fogFound&&fogBookMesh.parent!==groups['fog-room']){fogFound=true;controls['fog-return'].userData.action='OPEN DOOR · RETURN TO LIBRARY';controls['fog-return'].userData.author='The handle has appeared. Beyond the glass is warm library light.';notice('As you lift Lost in the Fog, a brass handle appears on the waiting-room door.',7)}if(zone!==lastStationZone){lastStationZone=zone;nextStationCue=t+35}if(active&&(zone==='signal-platform'||zone==='signal-room'||zone==='tide-platform'||zone==='tide-room')&&t>=nextStationCue){stationCue(zone);nextStationCue=t+42}if(zone!=='carriage'&&travelling)cancel();if(!travelling||!active)ride(0);if(travelling&&active){elapsed+=dt;if(looping&&elapsed>24)elapsed=8;/* on the loop the train holds its cruising speed */const speed=speedAt(elapsed);travelDistance+=speed*dt;journeyHud.update(elapsed,false,looping);ride(speed);if(elapsed>=RUN){travelling=false;stopIndex=targetStop;arrivalPending=false;alightCleared=true;arrived=arrived||stopIndex===4;fogArrived=stopIndex===3;controls.alight.userData.action='ALIGHT AT '+stops[stopIndex].name.toUpperCase();controls.alight.userData.author='The conductor has opened the vestibule door.';journeyHud.update(RUN,true);later(()=>journeyHud.hide(),6000);if(steam)steam();else sound(180,1.1,'sawtooth',.02);notice('“'+stops[stopIndex].name+'!” The brakes sigh and the train stands beneath the station lamps. The door at the rear of the carriage is open.',9)}}
      ...
      if(rainGlass){rainGlass.uniforms.uTime.value=reduced?0:t;rainGlass.uniforms.uSpeed.value=reduced?0:moving}
      if(passLight){if(passStart<0&&moving>.6&&!reduced&&t>=nextPassAt){passStart=t;passSide=Math.random()<.5?-1:1;nextPassAt=t+6+Math.random()*9}
        if(passStart>=0){const k=(t-passStart)/1.3;if(k>=1){passStart=-1;passLight.intensity=0}else{passLight.position.set(220.5+passSide*3.1,2.7,-8-k*24);passLight.intensity=Math.sin(Math.PI*k)*16}}else passLight.intensity=0}
    ...
    return {build,interact,update,allowed,clearLine,zoneAt,regions,groups,controls,entrance,cancel,get travelling(){return travelling},get looping(){return looping},get arrived(){return arrived},get fogArrived(){return fogArrived},get fogFound(){return fogFound},get built(){return built}};
  };
})();
