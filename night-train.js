...
(()=>{
  ...
  window.createNightTrain=function({ride=()=>{},whistle=null,steam=null,THREE,scene,MAT,player,collider,colliders,nearbyColliders=(x,z)=>colliders,interactables,canvasTexture,wrapText,coverTexture,books,fogBook=window.ATHENAEUM_FOG_BOOK,performanceZones,rememberLights,move,notice,home,sound:playCue=()=>{},modelTemplate,isLowBandwidth,registerSeat,camera,chooseDestination}){
    ...
    function update(t,dt,reduced,active,sound){sync();const zone=zoneAt(player.pos.x,player.pos.z)?.key;if(!zone)return false;if(fogBookMesh&&!fogFound&&fogBookMesh.parent!==groups['fog-room']){fogFound=true;controls['fog-return'].userData.action='OPEN DOOR · RETURN TO LIBRARY';controls['fog-return'].userData.author='The handle has appeared. Beyond the glass is warm library light.';notice('As you lift Lost in the Fog, a brass handle appears on the waiting-room door.',7)}if(zone!==lastStationZone){lastStationZone=zone;nextStationCue=t+35}if(active&&(zone==='signal-platform'||zone==='signal-room'||zone==='tide-platform'||zone==='tide-room')&&t>=nextStationCue){stationCue(zone);nextStationCue=t+42}if(zone!=='carriage'&&travelling)cancel();if(!travelling||!active)ride(0);if(travelling&&active){elapsed+=dt;const speed=speedAt(elapsed);travelDistance+=speed*dt;journeyHud.update(elapsed,false);ride(speed);if(elapsed>=RUN){travelling=false;stopIndex=targetStop;arrivalPending=false;alightCleared=true;arrived=arrived||stopIndex===4;fogArrived=stopIndex===3;controls.alight.userData.action='ALIGHT AT '+stops[stopIndex].name.toUpperCase();controls.alight.userData.author='The conductor has opened the vestibule door.';journeyHud.update(RUN,true);later(()=>journeyHud.hide(),6000);if(steam)steam();else sound(180,1.1,'sawtooth',.02);notice('“'+stops[stopIndex].name+'!” The brakes sigh and the train stands beneath the station lamps. The door at the rear of the carriage is open.',9)}}
