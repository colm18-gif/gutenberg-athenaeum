...
(function(){
  ...
  window.createRoomAmbience=function({audioCtx,master}){
    ...
    const EVENTS={
      ...
      creak:()=>buffer(.9,(d,n)=>{let next=0,ring=0,phase=0;for(let i=0;i<n;i++){const t=i/sr,env=Math.sin(Math.PI*t/.9);if(i>=next){ring=1;next=i+Math.floor(sr/(55+95*t/.9+Math.random()*12))}ring*=.992;phase+=2*Math.PI*(520+t*140)/sr;d[i]=Math.sin(phase)*ring*env*.5}}),
